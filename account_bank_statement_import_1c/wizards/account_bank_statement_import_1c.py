...
class AccountBankStatementImport(models.TransientModel):
    ...
    def _parse_file(self, data_file):
        ...
        for line_1c in sections_1c_lines:
            ...
            if line_1c.startswith('Конец'):
                ...
                if section_name == 'РасчСчет':
                    ...
                    raise UserError(
                        _('Unknown section type: %s.') % section_name)
                section_name = ''
        ...
        date_start = datetime.strptime(bank_statements[0]['ДатаНачала'],
                                       '%d.%m.%Y').date()
        balance_start = float(bank_statements[0][
                                  'НачальныйОстаток'])  # -o 'balance_start': float (e.g: 8368.56)
        balance_stop = float(bank_statements[-1][
                                 'КонечныйОстаток'])  # -o 'balance_end_real': float (e.g: 8888.88)
        ...
        for transaction_1c in transactions_1c:
            if len(transaction_1c) > 0:
                ...
                vals_line = {
                    ...
                    # 'date': date
                    'date': datetime.strptime(transaction_1c['Дата'],
                                              '%d.%m.%Y').date()
                }
                # 'amount': float
                if transaction_1c['ПлательщикРасчСчет'] == account_number:
                    vals_line['amount'] = -float(transaction_1c['Сумма'])
                    vals_line['account_number'] = transaction_1c[
                        'ПолучательРасчСчет']  # -o 'account_number': string
                    vals_line['partner_name'] = transaction_1c[
                        'Получатель']  # -o 'partner_name': string
                    # 'unique_import_id': string
                    vals_line['unique_import_id'] = '{}{}{}'.format(
                        transaction_1c['ПолучательРасчСчет'],
                        transaction_1c['Дата'].replace('.', ''),
                        transaction_1c['Номер'])
                else:
                    vals_line['amount'] = float(transaction_1c['Сумма'])
                    vals_line['account_number'] = transaction_1c[
                        'ПлательщикРасчСчет']  # -o 'account_number': string
                    vals_line['partner_name'] = transaction_1c[
                        'Плательщик']  # -o 'partner_name': string
                    # #- 'unique_import_id': string
                    vals_line['unique_import_id'] = '{}{}{}'.format(
                        transaction_1c['ПлательщикРасчСчет'],
                        transaction_1c['Дата'].replace('.', ''),
                        transaction_1c['Номер'])
                # -o 'note': string
                vals_line['ref'] = 'п/п № {}'.format(
                    transaction_1c['Номер'])  # -o 'ref': string
                transactions.append(vals_line)
        vals_bank_statement = {
            # 'name': string (e.g: '000000123')
            'name': 'from ' + common_info_1c['ДатаНачала'] + ' to ' +
                    common_info_1c['ДатаКонца'],
            'balance_start': balance_start,
            'date': date_start,  # 'date': date (e.g: 2013-06-26)
            'balance_end_real': balance_stop,
            'transactions': transactions
        }
        return None, account_number, [vals_bank_statement]

    def _check_acc_number_matching(self, account_number, journal_id):
        """Check if account number matches journal account number."""
        return journal_id.bank_account_id.acc_number == account_number
    ...
    def _complete_stmts_vals(self, stmt_vals, journal_id, account_number):
        ...
        if not self._check_acc_number_matching(account_number, journal_id):
            raise UserError(_("No account number found matching '%s' in journal '%s'.") % (account_number, journal_id.name))
